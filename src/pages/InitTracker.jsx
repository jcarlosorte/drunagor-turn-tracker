  ...
  const renderSlot = (index) => {
    ...
    const renderStack = (items, isTop, type = 'hero') => {
      ...
      return reversed.map((item, i) => {
        ...
        return (
          <div key={ type === 'enemy' ? item.enemy.uuid : type === 'rune' ? item.uuid : item.id } className={`absolute transition-transform duration-300 ${isCurrentTurn ? 'ring-4 ring-blue-500 rounded shadow-xl' : ''}`} style={{
              ...style,
