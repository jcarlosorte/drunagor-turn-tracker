...
const InitTracker = () => {
  ...
  const aplicarCapacidadesCartaEspecial = (cartaEspecial, placedEnemies) => {
    ...
    capacidades.forEach(cap => {
      if (cap.startsWith("SANA")) {
        // 🔍 Extraer partes: SANA_2_X_? o SANA_X_SI
        ...
        if (curacion <= 0) return; // No hacemos nada si es 0
  
        // ✅ Preguntar si tiene condición
        if (tieneCondicion) {
          const confirmar = window.confirm(`¿Aplicar efecto de SANA ${curacion}?`);
          if (!confirmar) return;
        }
  
        // ✅ Buscar enemigo por UUID y actualizar con updateEnemyVida
        if (cartaEspecial.sourceEnemyUUID) {
          ...
          if (enemigo) {
            nuevaVida = enemigo.enemy.vida + curacion;
            nuevoMax = enemigo.enemy.vidaMax;
        
            if (puedeSobrepasar) {
              // Puede aumentar vida máxima
              if (nuevaVida > enemigo.enemy.vidaMax) {
                nuevoMax = nuevaVida;
              }
            } else {
  ...
  antes de continuar he visto un fallo en el control de los turnos. Cuando se elimina una entidad, si hay más dentro del grupo, pasa el turno sin problema, pero cuando el turno tiene que pasar a otro grupo de enemigo, runa o heroe, no cambia de turno del todo y tengo como que pasarlo manualmente con el botón de siguiente turno para que llegue bien al que es. Te paso el código como esta actualemnte:
const [turnIndex, setTurnIndex] = useState(-1);
  const [currentTurnEntity, setCurrentTurnEntity] = useState(null);
  const [groupTurnTracker, setGroupTurnTracker] = useState({ group: [], index: 0 });
  const processedVoragineRef = useRef(new Set());
