 ...
 const CharacterCard = ({ name, image, position }) => (
  ...
  const EnemyCard = ({ name, comportamiento, categoria, image, position, uuid, color, onRemove }) => (
    ...
    >
      ...
      <div className="relative w-full max-h-[6rem] rounded-lg shadow-[0_6px_12px_rgba(0,0,0,0.5)]">
        <img
          src={image}
          alt={name}
          className={`w-full h-full object-cover rounded-lg border-2 ${borderColorMap[color] || ''}`}
        />
        <div
